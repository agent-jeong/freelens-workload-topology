...
- **Pod 로그** — Live tail, 이전 로그, 컨테이너 필터, 심각도 필터, 키워드 검색, 줄바꿈, 중복 숨김, 적응형 가상 스크롤링
...
현재 `src/renderer/index.tsx`는 의도적으로 작게 유지되며, 대부분의 구현은 `pages/`, `components/`, `topology/`, `utils/` 아래에 위치합니다.
